...
if [ "$#" -ne 9 ] && [ "$#" -ne 12 ]; then
    echo "Usage: $0 <APP_NAME> <CONTAINER_PORT> <DOCKER_IMAGE> <ENV_FILE> <CONTROLLER_TEMPLATE> <CONTROLLER_TYPE> <REPLICA_COUNT> <MANIFESTS_PATH> <IS_DATABASE> <MOUNT_PATH_DATA> <MOUNT_PATH_INIT_DB> <SUB_PATH_INIT_DB>"
    exit 1
fi
