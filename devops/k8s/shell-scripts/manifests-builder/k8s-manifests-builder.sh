...
./k8s-app-manifest-builder.sh $APP_VARIABLES_PATH $CONFIG_MAP_TEMPLATE $APP_SERVICE_TEMPLATE $APP_CONTROLLER_TEMPLATE $K8S_APP_PARAMETERS_CSV $CONTAINERS_CSV $MANIFESTS_PATH
...
./k8s-db-manifest-builder.sh $DB_RESOURCES_PATH $SECRET_TEMPLATE $PV_TEMPLATE $PVC_TEMPLATE $CONFIG_MAP_TEMPLATE $DB_SERVICE_TEMPLATE $DB_CONTROLLER_TEMPLATE $K8S_DB_PARAMETERS_CSV $CONTAINERS_CSV $MANIFESTS_PATH